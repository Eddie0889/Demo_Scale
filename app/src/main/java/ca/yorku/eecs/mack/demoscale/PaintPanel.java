...
public class PaintPanel extends View
{
    ...
    //*************************************************************************************
    //Double tap counter to determine if the image should be increased by a factor of 3 when
    //doubleTapCounter % 2 == 0 else  decrease  by factor of 3
    //*************************************************************************************
    public int doubleTapCounter;


    //********************************************
    //The names of these variables are obvious in what they store.
    //The reason for needing them is for the original feature
    //where the image cannot be flicked off the screen. The use
    //of these global variables aid in that.
    //********************************************
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        canvas.restore();
        System.out.println("this is the width of the canvas: " + canvas.getWidth());

        //*******************************
        //initialization of canvas width/height
        //reasons for needing these variables are stated where
        //the global variable is stated
        //*******************************
        ...
        switch (action & MotionEvent.ACTION_MASK)
        {
            ...
            case MotionEvent.ACTION_DOWN:
            {
                ...
                if (tapIsInImage(me))
                {
                    ...
                    lastTouchY = me.getY();
                    /*
                    xPosition = lastTouchX;
                    yPosition = lastTouchY;
                    scaleFactor = 3f;*/
            ...
            case MotionEvent.ACTION_POINTER_UP:
            {
                ...
                //***************************************************************************
                //checks to make sure when the pointer is lifted the image is still on screen
                //***************************************************************************
                keepInBounds();
                break;
            }
    ...
    //**********************************************************************************
    /*
    This Method ensures that the image stays on the screen in case if it get flicked off
     */
    //**********************************************************************************
    public void keepInBounds(){
       float rightSideOfImagePos = (xPosition + imageIntrinsicWidth) * scaleFactor;
       float leftSideOfImagePos = xPosition;
       float topOfImagePos = yPosition;
       float bottomOfImagePos = (yPosition + imageIntrinsicHeight) * scaleFactor;

        //checks if the right side of the image has gone off the left side of the screen
        // uses a buffer of 50 pixels so the user still has a bit of the image to touch
        if(rightSideOfImagePos < 50){
            xPosition = 0;
        }

        //this checks if the left side of the image goes off the right side of the screen
        // uses a buffer of 50 pixels so the user still has a bit of the image to touch
        if(leftSideOfImagePos > canvasWidth - 50 ){
            xPosition = canvasWidth / 3;
        }
        if(topOfImagePos > canvasHeight ){
            yPosition = canvasHeight - 300;

        }
        if (bottomOfImagePos < 50){
            yPosition=0;
        }

    }
    //**************************************************
    //method to detect if the tap is in the image or not.
    //Code cut from inside onTouchEvent() and paste here
    //for better code reuse.
    //**************************************************
    public boolean tapIsInImage(MotionEvent me){
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        //***********************************************************
        /*Overrides the onDoubleTap method which is a method in the
        GestureDetector.SimpleOnGestureListener API to handle when a
        user double taps on the image.
         */
        //***********************************************************
        @Override
        public boolean onDoubleTap(MotionEvent me){
            ...
            if (tapIsInImage(me)) {
                ...
                /*xPosition = me.getX() - canvasWidth;
                yPosition = me.getY() - canvasHeight;*/
                doubleTapCounter ++;
            }
